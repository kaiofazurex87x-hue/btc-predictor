...
class WhaleTracker:
    ...
    def track(self):
        try:
            ...
            whales        = all_orders[all_orders['value'] >= 500_000]
            for _, row in whales.iterrows():
                self.trades.append({'ts': datetime.now(), 'type': row['type'], 'value': row['value']})
            if whales.empty:
                return {'active': False, 'sentiment': 0, 'buy_vol': 0, 'sell_vol': 0, 'accumulating': False}
            buy_v  = whales[whales['type']=='buy']['value'].sum()
            sell_v = whales[whales['type']=='sell']['value'].sum()
            total  = buy_v + sell_v
            sent   = round((buy_v - sell_v) / total * 100, 1) if total else 0
            return {
                'active':      True,
                'sentiment':   sent,
    ...
    def get_signal(self):
        ...
        tot  = buy + sell
        return round((buy - sell) / tot * 100, 1) if tot else 0
